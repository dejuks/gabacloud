...
<div class="max-w-lg">
    ...
    <div class="bg-white rounded-2xl shadow p-6">
        <h2 class="font-semibold text-lg mb-4">Edit: {{ $category->name }}</h2>
        @if($errors->any())
        ...
        <form method="POST" action="{{ route('admin.categories.update', $category) }}" class="space-y-4">
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                    class="w-full border rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-indigo-400 focus:outline-none">{{ old('description', $category->description) }}</textarea>
            </div>
